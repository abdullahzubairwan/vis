...
# 3.0 Data
event_path = data.frame(event = list.files("data",pattern = "fm"))
...
teks_df = data.frame(event = c("twincity","kuching","borneo","score","melaka","klscm"),
                     lokasi = c("Cyberjaya-Putrajaya","Kuching","Kota Kinabalu","Putrajaya","Melaka","Kuala Lumpur"),
                     eg = c(232,46,90,275,126,485), # Filtered Elevation Gain
                     x = rep(36,6),
                     y = rep(87,6))
...
event_df = left_join(event_df,teks_df) %>% #left_join(.,ele_df) %>% 
  mutate(event = factor(event,levels = c("twincity","kuching","borneo","score","melaka","klscm")))

# 4.0 Labels
title = "Course Profile of <span style = 'color: #14213d;'>Malaysia's </span><br><span style = 'color: purple;'>Full Marathon</span> in 2023"
subtitle = "As running events are starting to get more popular across the country,<br>we take a look at the elevation on some of the selected Full Marathon events.<br>"
